...
const RightSide = () => {
  const { auth, message, homeUsers, theme, socket } = useSelector(state => state);
  const users = homeUsers.users;

  const dispatch = useDispatch();
  const { id } = useParams();
  const history = useHistory();

  // ✅ user se obtiene dinámicamente del estado global
  const user = useMemo(() => {
  ...
  const handleChangeMedia = (e) => {
    ...
    files.forEach(file => {
      ...
      if (file.size > 1024 * 1024 * 5) {
        return (err = 'El archivo debe pesar máximo 5MB.');
      }
  ...
  return (
    <>
        ...
        {user && (
          <UserCard user={user}>
            <div>
              ...
              <i className="fas fa-trash text-danger mr-1" onClick={handleDeleteConversation} />
            </div>
          </UserCard>
        )}
      </div>
      ...
      <form className="chat_input" onSubmit={handleSubmit}>
        ...
        {/* ✅ Input visible solo si el usuario tiene permiso */}
        {user?.opcionesUser?.includes('archivos') && (
